...
$\color{red}{注：}$可能需要查加法器、乘法器的组合逻辑实现，而非直接使用verilog中的'+','*'的操作符

Host内部设有一个程序指针，名为PC，指向当前指令在指令栈中的地址。如果当前指令是分支跳转指令，则跳转到相应地址，否则PC+=1。

$\color{skyblue}{之所以做这个稍微有点复杂的处理器，是为了后面DE-SoC实验能够复用}$
...
$\color{red}{注：}$写程序的时候可以手撕机器码，不着急做编译器。
...
$\color{#FF0000}{注：}$此处双时钟设计可能会冲突报错。先进行Buffer的双时钟设计以确保此设计可行。
